package org.gnor.rocketmq.cretry_rNPC_14.broker.store;
...
public class MessageStore {
    ...
    public static class StoredMessage {
        ...
        public StoredMessage(String topic, String body, String status, int queueId, Map<String, String> properties) {
            this.topic = topic;
            this.body = body;
            this.status = status;
            this.queueId = queueId;
    ...
    public StoredMessage getMessage(int pos, int size) {
        ...
        short topicSize = readBuffer.getShort();
        byte[] topicData = new byte[topicSize];
        readBuffer.get(topicData);
        short propertiesSize = readBuffer.getShort(); // Convert to unsigned
        ...
        System.out.println("propertiesSize: " + propertiesSize);
    ...
    public DispatchRequest checkMessageAndReturnSize(ByteBuffer readBuffer) {
        ...
        String tags = propertiesMap.get("TAG");
        if (null != tags && !tags.isEmpty()) {
            tagsCode = tags.hashCode();
        }
        {
            String t = propertiesMap.get("DELAY");
            if ("SCHEDULE_TOPIC_XXXX".equals(new String(topicData)) && null != t) {
                int delayLevel = Integer.parseInt(t);
                if (delayLevel > 0) {
                    Long time = this.delayLevelTable.get(delayLevel);
                    if (time != null) {
                        tagsCode = time + storeTimestamp;
                    } else {
                        tagsCode = storeTimestamp + 1000;
                    }
                }
            }

        }
        return new DispatchRequest(new String(topicData), queueId, physicOffset, totalSize, tagsCode, propertiesMap);
    }
